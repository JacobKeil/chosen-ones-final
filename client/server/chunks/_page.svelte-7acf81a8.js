...
const image5 = "/_app/immutable/assets/banner_pic (5)-363bf4bc.jpg";
...
const Banner = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<section id="${"banner"}" class="${"relative h-md text-black bg-[#fff] overflow-hidden"}">
  <div id="${"carousel-left"}" class="${"absolute flex justify-center items-center top-[47%] left-4 p-1 z-20 rounded-full bg-white/75 cursor-pointer"}"><box-icon class="${"fill-black cursor-pointer"}" name="${"chevron-left"}"></box-icon></div>
  
  <div id="${"carousel-right"}" class="${"absolute flex justify-center items-center top-[47%] right-4 p-1 z-20 rounded-full bg-white/75 cursor-pointer"}"><box-icon class="${"fill-black cursor-pointer"}" name="${"chevron-right"}"></box-icon></div>
  <div id="${"image1"}" class="${"absolute object-cover h-full w-full"}"><img class="${"z-10 object-cover h-full w-full blur-sm scale-[1.1]"}"${add_attribute("src", images[currentImage], 0)} alt="${""}"></div>
  <div id="${"image"}" class="${"absolute object-cover h-full w-full"}"><img class="${"object-cover z-20 mobile:object-contain left-0 right-0 mx-auto h-full"}"${add_attribute("src", images[currentImage], 0)} alt="${""}"></div></section>`;
});
...
<section id="${"store"}" class="${"bg-black overflow-hidden"}"><div class="${"p-8 text-[#fff]"}"><p class="${"text-2xl"}">Store</p></div>
  <div class="${"px-8 grid grid-cols-1 mobile:grid-cols-2 tablet:grid-cols-3 gap-4"}">${each($products, (product, index) => {
    return `${productNum.length < 3 ? `${validate_component(Product, "Product").$$render($$result, { product }, {}, {})}
...
<section id="${"blog"}" class="${"relative bg-white overflow-hidden"}"><div class="${"p-8"}"><p class="${"text-2xl"}">Blog</p></div>
  <div class="${"px-8 grid grid-cols-1 mobile:grid-cols-2 tablet:grid-cols-3 gap-4"}">${each($articles, (article, index) => {
    return `${article.tags.includes("Article") && blogNum.length < 3 ? `${validate_component(Article, "Article").$$render($$result, { article }, {}, {})}
